...
import { initSectionComponents } from "./components/sections/index.js?v=20260807-timelinefocus";
...
function initSafely(label, initializer, fallback) {
  try {
    return initializer();
  } catch (error) {
    console.error(`[app] ${label} 初始化失敗，保留靜態內容。`, error);
    return fallback;
  }
}

function recoverFromLoadingFailure() {
  document.body.classList.remove("is-loading");
  document.querySelector("[data-loader]")?.setAttribute("aria-hidden", "true");
}
...
function updateRuntimeStatus(scrollMode, sections) {
  ...
  const sectionCount = sections.filter((section) => section?.mode === "gsap").length;
  status.textContent = scrollMode === "static" ? "靜態閱讀模式已就緒" : `${sectionCount} 個章節動畫已啟動`;
}

// 應用程式入口只負責組裝模組，頁面內容應由未來的業務模組自行掛載。
function bootstrap() {
  initSafely("dependency status", publishDependencyStatus, undefined);
  initSafely("theme", initTheme, undefined);
  initSafely("loading", initLoading, recoverFromLoadingFailure);
  initSafely("page transition", initPageTransition, undefined);
  initSafely("progress navigation", initProgressNavigation, undefined);
  initSafely("scroll snap", initScrollSnap, undefined);
  const smoothScroll = initSafely("smooth scroll", initSmoothScroll, { mode: "static" });
  initSafely("evidence fallback", initEvidenceFallback, undefined);
  const sections = initSafely("section components", initSectionComponents, []);
  const scrollEffects = initSafely("scroll effects", initScrollEffects, { mode: "static" });
  const presentation = initSafely("presentation mode", initPresentationMode, undefined);
  const sceneInteractions = initSafely("scene interactions", initSceneInteractions, undefined);
