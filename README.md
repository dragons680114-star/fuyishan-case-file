...
這會檢查 HTML、CSS 與 JavaScript 模組的本機資源引用是否存在、各頁的標題／description／canonical 是否齊全、`sitemap.xml` 與 `site.webmanifest` 是否有效、HTML/CSS 是否重新引入 CDN，並列出 `assets/` 中內容完全重複的檔案。
...
- 桌機預設以 16:9 全螢幕畫布呈現，每個主要 Section 會變成一頁可切換的展示畫面。
- 可使用右上角「展示模式」、左右方向鍵、PageUp / PageDown、滑鼠滾輪、底部圓點與上一頁／下一頁控制。
- 可由展示控制列進入瀏覽器全螢幕；離開後仍可回到一般長頁閱讀模式。
- Tablet 與 Mobile 保留自然滾動閱讀，避免小螢幕被 16:9 畫布裁切。
- 展示模式偏好會記錄在瀏覽器的 localStorage；日後若更換簡報內容，不需要修改動畫控制邏輯。
